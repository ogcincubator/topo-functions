...
| `CC-07`            | Containment and host topology  | `TR-09`, `TR-20`, `TR-21`, `TR-28`, `TR-29`          |
...
**2D data.** The conformance classes above assume 3D coordinates, but a dataset may legitimately mix 2D content (e.g. a cadastral parcel outline) with 3D topology, or be entirely 2D. Neither case is a structural failure: a point is validated against its own minimum of 2 coordinate values, not one dataset-wide length, so a 2D point is never flagged `INVALID_COORDINATES` merely because the rest of the dataset is 3D (or vice versa). A CSDM `parcels` collection (`topology.type: "Polygon"`) is parsed into a real `Surface`, chaining its ordered curve ids into a closed ring the same way `topo2geojson` resolves the same reference shape into geometry. Points, curves, and surfaces that are 2D are excluded from the 3D-specific conformance classes and instead validated by the 2D-applicable subset of point/curve/surface rules (TR-01, TR-03, TR-04, TR-05, TR-11, TR-12, TR-13, TR-16, TR-17, TR-22, TR-23, TR-02, TR-14, TR-15) under the same issue codes, tagged `extra.dimensionality: "2d"`; text/HTML/JSON reports render these in their own section/key, separate from the main rule-results table. Shell/solid/volume-specific rules (TR-06 onward) have no 2D analogue and simply see nothing to check for 2D content — a solid excluded from 3D validation purely because it owns 2D-tainted geometry produces a summarized, non-silent `EXCLUDED_FROM_3D_VALIDATION` notice instead. A dataset whose points are *all* 2D additionally produces a single `NO_3D_TOPOLOGY` warning; `valid`/the CLI exit code stay `0` unless a genuine defect is found (an error-severity issue), not merely because the dataset is 2D. TR-28/TR-29 separately check a *declared* solid-to-parcel relationship (`topology.relationships`, role `containingPrimaryParcel`/`burdenedBySecondaryParcel`) against the parcel treated as an unlimited vertical prism — see `topo_validator/topology_rules.md` for the declaration shape. Full 2D/2.5D *parcel-fabric* coverage — primary-parcel overlap and contiguity, the 2D analogue of TR-08 — remains a future extension (see `topo_validator/topology_rules.md`'s "Rules Not Yet Implemented" section).
